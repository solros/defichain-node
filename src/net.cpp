...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Defi users should be well used to small numbers with many decimal places by now :)
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string &seed : vSeeds) {
        if (interruptNet) {
            LogPrintf("net:::Interrupt net\n");
            return;
        }
        if (HaveNameProxy()) {
            LogPrintf("net:::HaveNameProxy\n");
            AddOneShot(seed);
        } else {
            ...
            if (!resolveSource.SetInternal(host)) {
                LogPrintf("net:::ResolveSourceInternal - continue\n");
                continue;
            }
            unsigned int nMaxIPs = 256; // Limits number of IPs learned from a DNS seed
            LogPrintf("net:::Lookup\n");
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true))
            {
                LogPrintf("net:::Adding %d addresses\n", vIPs.size());
                for (const CNetAddr& ip : vIPs) {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip, Params().GetDefaultPort()), requiredServiceBits);
                    addr.nTime = GetTime() - 3*nOneDay - GetRand(4*nOneDay); // use a random age between 3 and 7 days old
                    vAdd.push_back(addr);
                    LogPrintf("net:::Adding addr: %s\n", addr.ToStringIPPort());
                    found++;
                }
                addrman.Add(vAdd, resolveSource);
            } else {
                LogPrintf("net:::OneShot\n");
